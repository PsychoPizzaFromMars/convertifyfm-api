...
# TODO: - change credentials implementation on os.environ() or smth like that
#       - change returned values of request methods to response instead of tuple
#       - .env version of storing credentials -- https://www.realpythonproject.com/3-ways-to-store-and-read-credentials-locally-in-python/
#       - relocate _restructure into method, because it's not universal
#       - best way to call API (handling exceptions and validation)
...
class LastFM_API:
    ...
    def get_top_tracks(self, user_id='androw-go-go', period='overall', limit='1000', page='1'):
        params = {
        ...
        toptracks_list = [
            f"{elem['artist']['name']} - {elem['name']}" for elem in response['toptracks']['track']]
        return toptracks_list

    def get_weekly_trackchart(self, user_id='androw-go-go', date_from=946674000, date_to=int(_unixtimestamp_now()), limit='200', page='1'):
        params = {
        ...
        if "error" in response:
            return []
        tracklist = [
            f"{elem['artist']['#text']} - {elem['name']}" for elem in response['weeklytrackchart']['track']]
        return tracklist
...
class Spotify_API:
    ...
    def get_tokens(self, code):
        ...
        auth_header = base64.standard_b64encode(
            str(precodedclient).encode()).decode()
        header = {
    ...
    def get_users_top_tracks(self, access_token, type="tracks", time_range="medium_term", limit=10, offset=0):
